...
class F1_Evaluator:
    ...
    def calculate_f1_list(self, data: List[Dict[str, str]], predictions: List[str]) -> float:
        predictions = clean_answer_list(predictions)
        ...
        if len(data) != len(predictions):
            logger.error("data:"+str(len(data))+". predictions:"+str(len(predictions)))
...
def clean_answer_list(answers: List[Optional[str]]) -> List[Optional[str]]:
    """清洗答案列表，但保留 None 的位置，避免长度改变"""
    return [clean_answer(ans) if ans is not None else None for ans in answers]

def clean_answer(text: str) -> str:
    """
    去除答案中的中括号 [] 和小括号 ()，并去除多余空格。
    如 "[No]"、"(Yes)" → "No", "Yes"
    """
